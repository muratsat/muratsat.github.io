...
> _A word of warning: `poll()` is horribly slow when it comes to giant numbers of connections. In those circumstances, you’ll get better performance out of an event library such as [libevent](https://libevent.org/)[27](#fn27) that attempts to use the fastest possible method availabile on your system._
...
(There’s actually more that you can do with the `poll()` call. See the [`poll()` man page, below](#pollman), for more details.)
...
> _A word of warning: `select()`, though very portable, is terribly slow when it comes to giant numbers of connections. In those circumstances, you’ll get better performance out of an event library such as [libevent](https://libevent.org/)[30](#fn30) that attempts to use the fastest possible method availabile on your system._
...
Just set `tv_sec` to the number of seconds to wait, and set `tv_usec` to the number of microseconds to wait. Yes, that’s \_micro\_seconds, not milliseconds. There are 1,000 microseconds in a millisecond, and 1,000 milliseconds in a second. Thus, there are 1,000,000 microseconds in a second. Why is it “usec”? The “u” is supposed to look like the Greek letter μ (Mu) that we use for “micro”. Also, when the function returns, `timeout` _might_ be updated to show the time still remaining. This depends on what flavor of Unix you’re running.
...
[The following code snippet](https://beej.us/guide/bgnet/examples/select.c)[31](#fn31) waits 2.5 seconds for something to appear on standard input:
...
[This program](https://beej.us/guide/bgnet/examples/selectserver.c)[32](#fn32) acts like a simple multi-user chat server. Start it running in one window, then `telnet` to it (“`telnet hostname 9034`”) from multiple other windows. When you type something in one `telnet` session, it should appear in all the others.
...
Quick note to all you Linux fans out there: sometimes, in rare circumstances, Linux’s `select()` can return “ready-to-read” and then not actually be ready to read! This means it will block on the `read()` after the `select()` says it won’t! Why you little—! Anyway, the workaround solution is to set the `O_NONBLOCK` flag on the receiving socket so it errors with `EWOULDBLOCK` (which you can just safely ignore if it occurs). See the [`fcntl()` reference page](#fcntlman) for more info on setting a socket to non-blocking.

In addition, here is a bonus afterthought: there is another function called `poll()` which behaves much the same way `select()` does, but with a different system for managing the file descriptor sets. [Check it out!](#pollman)

7.4 Handling Partial `send()`s
------------------------------

Remember back in the [section about `send()`](#sendrecv), above, when I said that `send()` might not send all the bytes you asked it to? That is, you want it to send 512 bytes, but it returns 412. What happened to the remaining 100 bytes?
...
The first method, encoding the numbers as text before sending, has the advantage that you can easily print and read the data that’s coming over the wire. Sometimes a human-readable protocol is excellent to use in a non-bandwidth-intensive situation, such as with [Internet Relay Chat (IRC)](https://en.wikipedia.org/wiki/Internet_Relay_Chat)[33](#fn33). However, it has the disadvantage that it is slow to convert, and the results almost always take up more space than the original number!
...
The thing to do is to pack the data into a known format and send that over the wire for decoding. For example, to pack `float`s, here’s [something quick and dirty with plenty of room for improvement](https://beej.us/guide/bgnet/examples/pack.c)[34](#fn34):
...
What can we do instead? Well, _The_ Standard for storing floating point numbers is known as [IEEE-754](https://en.wikipedia.org/wiki/IEEE_754)[35](#fn35). Most computers use this format internally for doing floating point math, so in those cases, strictly speaking, conversion wouldn’t need to be done. But if you want your source code to be portable, that’s an assumption you can’t necessarily make. (On the other hand, if you want things to be fast, you should optimize this out on platforms that don’t need to do it! That’s what `htons()` and its ilk do.)

[Here’s some code that encodes floats and doubles into IEEE-754 format](https://beej.us/guide/bgnet/examples/ieee754.c)[36](#fn36). (Mostly—it doesn’t encode NaN or Infinity, but it could be modified to do that.)
...
In the book [_The Practice of Programming_](https://beej.us/guide/url/tpop)[37](#fn37) by Kernighan and Pike, they implement `printf()`\-like functions called `pack()` and `unpack()` that do exactly this. I’d link to them, but apparently those functions aren’t online with the rest of the source from the book.

(The Practice of Programming is an excellent read. Zeus saves a kitten every time I recommend it.)

At this point, I’m going to drop a pointer to a [Protocol Buffers implementation in C](https://github.com/protobuf-c/protobuf-c)[38](#fn38) which I’ve never used, but looks completely respectable. Python and Perl programmers will want to check out their language’s `pack()` and `unpack()` functions for accomplishing the same thing. And Java has a big-ol’ Serializable interface that can be used in a similar way.

But if you want to write your own packing utility in C, K&P’s trick is to use variable argument lists to make `printf()`\-like functions to build the packets. [Here’s a version I cooked up](https://beej.us/guide/bgnet/examples/pack2.c)[39](#fn39) on my own based on that which hopefully will be enough to give you an idea of how such a thing can work.
...
And so on. How does the client know when one message starts and another stops? You could, if you wanted, make all messages the same length and just call the `sendall()` we implemented, [above](#sendall). But that wastes bandwidth! We don’t want to `send()` 1024 bytes just so “tom” can say “Hi”.
...
When you’re sending this data, you should be safe and use a command similar to [`sendall()`](#sendall), above, so you know all the data is sent, even if it takes multiple calls to `send()` to get it all out.
...
So what happens if you try to send data on the broadcast address without first setting the `SO_BROADCAST` socket option? Well, let’s fire up good old [`talker` and `listener`](#datagram) and see what happens.
...
In fact, that’s the _only difference_ between a UDP application that can broadcast and one that can’t. So let’s take the old `talker` application and add one section that sets the `SO_BROADCAST` socket option. We’ll call this program [`broadcaster.c`](https://beej.us/guide/bgnet/examples/broadcaster.c)[42](#fn42):
...
You have to use `setsockopt()` with the `SO_REUSEADDR` option on the listening socket. Check out the [section on `bind()`](#bind) and the [section on `select()`](#select) for an example.
...
All your raw sockets questions will be answered in [W. Richard Stevens’ UNIX Network Programming books](#books). Also, look in the `ping/` subdirectory in Stevens’ UNIX Network Programming source code, [available online](http://www.unpbook.com/src.html)[43](#fn43).

**How do I change or shorten the timeout on a call to `connect()`?**

Instead of giving you exactly the same answer that W. Richard Stevens would give you, I’ll just refer you to [`lib/connect_nonb.c` in the UNIX Network Programming source code](http://www.unpbook.com/src.html)[44](#fn44).

The gist of it is that you make a socket descriptor with `socket()`, [set it to non-blocking](#blocking), call `connect()`, and if all goes well `connect()` will return `-1` immediately and `errno` will be set to `EINPROGRESS`. Then you call [`select()`](#select) with whatever timeout you want, passing the socket descriptor in both the read and write sets. If it doesn’t timeout, it means the `connect()` call completed. At this point, you’ll have to use `getsockopt()` with the `SO_ERROR` option to get the return value from the `connect()` call, which should be zero if there was no error.
...
First, delete Windows and install Linux or BSD. `};-)`. No, actually, just see the [section on building for Windows](#windows) in the introduction.

**How do I build for Solaris/SunOS? I keep getting linker errors when I try to compile!**

The linker errors happen because Sun boxes don’t automatically compile in the socket libraries. See the [section on building for Solaris/SunOS](#solaris) in the introduction for an example of how to do this.
...
Use [`select()`](#select)! It allows you to specify a timeout parameter for socket descriptors that you’re looking to read from. Or, you could wrap the entire functionality in a single function, like this:
...
One easy way to do encryption is to use SSL (secure sockets layer), but that’s beyond the scope of this guide. (Check out the [OpenSSL project](https://www.openssl.org/)[45](#fn45) for more info.)
...
You have to make sure all the data is being sent, first of all. (See the [`sendall()`](#sendall) function implementation for details.) Once you’re sure of that, then you need to call `recv()` in a loop until all your data is read.

Read the section [Son of Data Encapsulation](#sonofdataencap) for details on receiving complete packets of data using multiple calls to `recv()`.
...
For those not in the know, when a network card is in “promiscuous mode”, it will forward ALL packets to the operating system, not just those that were addressed to this particular machine. (We’re talking Ethernet-layer addresses here, not IP addresses–but since ethernet is lower-layer than IP, all IP addresses are effectively forwarded as well. See the section [Low Level Nonsense and Network Theory](#lowlevel) for more info.)
...
Unfortunately, the answer to the question varies depending on the platform, but if you Google for, for instance, “windows promiscuous ioctl” you’ll probably get somewhere. For Linux, there’s what looks like a [useful Stack Overflow thread](https://stackoverflow.com/questions/21323023/)[46](#fn46), as well.
...
[`socket()`](#socketman), [`getaddrinfo()`](#getaddrinfoman), [`listen()`](#listenman), [`struct sockaddr_in`](#structsockaddrman)
...
[`getaddrinfo()`](#getaddrinfoman), [`socket()`](#socketman), [`struct sockaddr_in`](#structsockaddrman), [`struct in_addr`](#structsockaddrman)
...
The `servname` parameter is basically the port number. It can be a port number (passed as a string, like “80”), or it can be a service name, like “http” or “tftp” or “smtp” or “pop”, etc. Well-known service names can be found in the [IANA Port List](https://www.iana.org/assignments/port-numbers)[48](#fn48) or in your `/etc/services` file.
...
[`getaddrinfo()`](#getaddrinfoman), [`getnameinfo()`](#getnameinfoman), [`gethostname()`](#gethostnameman), [`errno`](#errnoman), [`perror()`](#perrorman), [`strerror()`](#perrorman), [`struct in_addr`](#structsockaddrman)
...
[`gethostname()`](#gethostnameman), [`gethostbyname()`](#gethostbynameman), [`gethostbyaddr()`](#gethostbynameman)
...
Set the socket to be non-blocking. See the section on [blocking](#blocking) for more details.
...
_These functions are deprecated because they don’t handle IPv6! Use (`inet_ntop()`)\[#inet\_ntopman\] or (`inet_pton()`)\[#inet\_ntopman\] instead! They are included here because they can still be found in the wild._
...
[`inet_ntop()`](#inet_ntopman), [`inet_pton()`](#inet_ntopman), [`gethostbyname()`](#gethostbynameman), [`gethostbyaddr()`](#gethostbynameman)
...
If you want to call `recv()` “just for pretend”, you can call it with this flag. This will tell you what’s waiting in the buffer for when you call `recv()` “for real” (i.e. _without_ the `MSG_PEEK` flag. It’s like a sneak preview into the next `recv()` call.
...
[`send()`](#sendman), [`sendto()`](#sendman), [`select()`](#selectman), [`poll()`](#pollman), [Blocking](#blocking)
...
Note for Linux users: Linux’s `select()` can return “ready-to-read” and then not actually be ready to read, thus causing the subsequent `read()` call to block. You can work around this bug by setting `O_NONBLOCK` flag on the receiving socket so it errors with `EWOULDBLOCK`, then ignoring this error if it occurs. See the [`fcntl()` man page](#fcntlman) for more info on setting a socket to non-blocking.
...
Allows UDP datagram (`SOCK_DGRAM`) sockets to send and receive packets sent to and from the broadcast address. Does nothing—_NOTHING!!_—to TCP stream sockets! Hahaha!
...
If `send()` would block because outbound traffic is clogged, have it return `EAGAIN`. This is like a “enable non-blocking just for this send.” See the section on [blocking](#blocking) for more details.
...
Returns the number of bytes actually sent, or `-1` on error (and `errno` will be set accordingly). Note that the number of bytes actually sent might be less than the number you asked it to send! See the section on [handling partial `send()`s](#sendall) for a helper function to get around this.
...
[`accept()`](#acceptman), [`bind()`](#bindman), [`getaddrinfo()`](#getaddrinfoman), [`listen()`](#listenman)
...
[`accept()`](#acceptman), [`bind()`](#bindman), [`connect()`](#connectman), [`inet_aton()`](#inet_ntoaman), [`inet_ntoa()`](#inet_ntoaman)
...
**Unix Network Programming, volumes 1-2** by W. Richard Stevens. Published by Addison-Wesley Professional and Prentice Hall. ISBNs for volumes 1-2: [978-0131411555](https://beej.us/guide/url/unixnet1)[49](#fn49), [978-0130810816](https://beej.us/guide/url/unixnet2)[50](#fn50).

**Internetworking with TCP/IP, volume I** by Douglas E. Comer. Published by Pearson. ISBN [978-0136085300](https://beej.us/guide/url/intertcp1)[51](#fn51).

**TCP/IP Illustrated, volumes 1-3** by W. Richard Stevens and Gary R. Wright. Published by Addison Wesley. ISBNs for volumes 1, 2, and 3 (and a 3-volume set): [978-0201633467](https://beej.us/guide/url/tcpi1)[52](#fn52), [978-0201633542](https://beej.us/guide/url/tcpi2)[53](#fn53), [978-0201634952](https://beej.us/guide/url/tcpi3)[54](#fn54), ([978-0201776317](https://beej.us/guide/url/tcpi123)[55](#fn55)).

**TCP/IP Network Administration** by Craig Hunt. Published by O’Reilly & Associates, Inc. ISBN [978-0596002978](https://beej.us/guide/url/tcpna)[56](#fn56).

**Advanced Programming in the UNIX Environment** by W. Richard Stevens. Published by Addison Wesley. ISBN [978-0321637734](https://beej.us/guide/url/advunix)[57](#fn57).
...
**[BSD Sockets: A Quick And Dirty Primer](https://cis.temple.edu/~giorgio/old/cis307s96/readings/docs/sockets.html)[58](#fn58)** (Unix system programming info, too!)

**[The Unix Socket FAQ](https://developerweb.net/?f=70)[59](#fn59)**

**[TCP/IP FAQ](http://www.faqs.org/faqs/internet/tcp-ip/tcp-ip-faq/part1/)[60](#fn60)**

**[The Winsock FAQ](https://tangentsoft.net/wskfaq/)[61](#fn61)**

And here are some relevant Wikipedia pages:

**[Berkeley Sockets](https://en.wikipedia.org/wiki/Berkeley_sockets)[62](#fn62)**

**[Internet Protocol (IP)](https://en.wikipedia.org/wiki/Internet_Protocol)[63](#fn63)**

**[Transmission Control Protocol (TCP)](https://en.wikipedia.org/wiki/Transmission_Control_Protocol)[64](#fn64)**

**[User Datagram Protocol (UDP)](https://en.wikipedia.org/wiki/User_Datagram_Protocol)[65](#fn65)**

**[Client-Server](https://en.wikipedia.org/wiki/Client-server)[66](#fn66)**

**[Serialization](https://en.wikipedia.org/wiki/Serialization)[67](#fn67)** (packing and unpacking data)

10.3 RFCs
---------

[RFCs](https://www.rfc-editor.org/)[68](#fn68)—the real dirt! These are documents that describe assigned numbers, programming APIs, and protocols that are used on the Internet. I’ve included links to a few of them here for your enjoyment, so grab a bucket of popcorn and put on your thinking cap:

**[RFC 1](https://tools.ietf.org/html/rfc1)[69](#fn69)** —The First RFC; this gives you an idea of what the “Internet” was like just as it was coming to life, and an insight into how it was being designed from the ground up. (This RFC is completely obsolete, obviously!)

**[RFC 768](https://tools.ietf.org/html/rfc768)[70](#fn70)** —The User Datagram Protocol (UDP)

**[RFC 791](https://tools.ietf.org/html/rfc791)[71](#fn71)** —The Internet Protocol (IP)

**[RFC 793](https://tools.ietf.org/html/rfc793)[72](#fn72)** —The Transmission Control Protocol (TCP)

**[RFC 854](https://tools.ietf.org/html/rfc854)[73](#fn73)** —The Telnet Protocol

**[RFC 959](https://tools.ietf.org/html/rfc959)[74](#fn74)** —File Transfer Protocol (FTP)

**[RFC 1350](https://tools.ietf.org/html/rfc1350)[75](#fn75)** —The Trivial File Transfer Protocol (TFTP)

**[RFC 1459](https://tools.ietf.org/html/rfc1459)[76](#fn76)** —Internet Relay Chat Protocol (IRC)

**[RFC 1918](https://tools.ietf.org/html/rfc1918)[77](#fn77)** —Address Allocation for Private Internets

**[RFC 2131](https://tools.ietf.org/html/rfc2131)[78](#fn78)** —Dynamic Host Configuration Protocol (DHCP)

**[RFC 2616](https://tools.ietf.org/html/rfc2616)[79](#fn79)** —Hypertext Transfer Protocol (HTTP)

**[RFC 2821](https://tools.ietf.org/html/rfc2821)[80](#fn80)** —Simple Mail Transfer Protocol (SMTP)

**[RFC 3330](https://tools.ietf.org/html/rfc3330)[81](#fn81)** —Special-Use IPv4 Addresses

**[RFC 3493](https://tools.ietf.org/html/rfc3493)[82](#fn82)** —Basic Socket Interface Extensions for IPv6

**[RFC 3542](https://tools.ietf.org/html/rfc3542)[83](#fn83)** —Advanced Sockets Application Program Interface (API) for IPv6

**[RFC 3849](https://tools.ietf.org/html/rfc3849)[84](#fn84)** —IPv6 Address Prefix Reserved for Documentation

**[RFC 3920](https://tools.ietf.org/html/rfc3920)[85](#fn85)** —Extensible Messaging and Presence Protocol (XMPP)

**[RFC 3977](https://tools.ietf.org/html/rfc3977)[86](#fn86)** —Network News Transfer Protocol (NNTP)

**[RFC 4193](https://tools.ietf.org/html/rfc4193)[87](#fn87)** —Unique Local IPv6 Unicast Addresses

**[RFC 4506](https://tools.ietf.org/html/rfc4506)[88](#fn88)** —External Data Representation Standard (XDR)

The IETF has a nice online tool for [searching and browsing RFCs](https://tools.ietf.org/rfc/)[89](#fn89).

* * *

1.  https://www.linux.com/[↩︎](#fnref1)
    
2.  https://bsd.org/[↩︎](#fnref2)
    
3.  https://learn.microsoft.com/en-us/windows/wsl/[↩︎](#fnref3)
    
4.  https://cygwin.com/[↩︎](#fnref4)
    
5.  https://www.tallyhawk.net/WinsockExamples/[↩︎](#fnref5)
    
6.  http://www.catb.org/~esr/faqs/smart-questions.html[↩︎](#fnref6)
    
7.  https://beej.us/guide/bgnet/examples/telnot.c[↩︎](#fnref7)
    
8.  https://tools.ietf.org/html/rfc854[↩︎](#fnref8)
    
9.  https://tools.ietf.org/html/rfc793[↩︎](#fnref9)
    
10.  https://tools.ietf.org/html/rfc791[↩︎](#fnref10)
    
11.  https://tools.ietf.org/html/rfc768[↩︎](#fnref11)
    
12.  https://tools.ietf.org/html/rfc791[↩︎](#fnref12)
    
13.  https://en.wikipedia.org/wiki/Vint\_Cerf[↩︎](#fnref13)
    
14.  https://en.wikipedia.org/wiki/ELIZA[↩︎](#fnref14)
    
15.  https://www.iana.org/assignments/port-numbers[↩︎](#fnref15)
    
16.  https://en.wikipedia.org/wiki/Doom\_(1993\_video\_game)[↩︎](#fnref16)
    
17.  https://en.wikipedia.org/wiki/Wilford\_Brimley[↩︎](#fnref17)
    
18.  https://tools.ietf.org/html/rfc1918[↩︎](#fnref18)
    
19.  https://tools.ietf.org/html/rfc4193[↩︎](#fnref19)
    
20.  https://www.iana.org/assignments/port-numbers[↩︎](#fnref20)
    
21.  https://beej.us/guide/bgnet/examples/showip.c[↩︎](#fnref21)
    
22.  https://tools.ietf.org/html/rfc1413[↩︎](#fnref22)
    
23.  https://beej.us/guide/bgnet/examples/server.c[↩︎](#fnref23)
    
24.  https://beej.us/guide/bgnet/examples/client.c[↩︎](#fnref24)
    
25.  https://beej.us/guide/bgnet/examples/listener.c[↩︎](#fnref25)
    
26.  https://beej.us/guide/bgnet/examples/talker.c[↩︎](#fnref26)
    
27.  https://libevent.org/[↩︎](#fnref27)
    
28.  https://beej.us/guide/bgnet/examples/poll.c[↩︎](#fnref28)
    
29.  https://beej.us/guide/bgnet/examples/pollserver.c[↩︎](#fnref29)
    
30.  https://libevent.org/[↩︎](#fnref30)
    
31.  https://beej.us/guide/bgnet/examples/select.c[↩︎](#fnref31)
    
32.  https://beej.us/guide/bgnet/examples/selectserver.c[↩︎](#fnref32)
    
33.  https://en.wikipedia.org/wiki/Internet\_Relay\_Chat[↩︎](#fnref33)
    
34.  https://beej.us/guide/bgnet/examples/pack.c[↩︎](#fnref34)
    
35.  https://en.wikipedia.org/wiki/IEEE\_754[↩︎](#fnref35)
    
36.  https://beej.us/guide/bgnet/examples/ieee754.c[↩︎](#fnref36)
    
37.  https://beej.us/guide/url/tpop[↩︎](#fnref37)
    
38.  https://github.com/protobuf-c/protobuf-c[↩︎](#fnref38)
    
39.  https://beej.us/guide/bgnet/examples/pack2.c[↩︎](#fnref39)
    
40.  https://beej.us/guide/bgnet/examples/pack2.c[↩︎](#fnref40)
    
41.  https://tools.ietf.org/html/rfc4506[↩︎](#fnref41)
    
42.  https://beej.us/guide/bgnet/examples/broadcaster.c[↩︎](#fnref42)
    
43.  http://www.unpbook.com/src.html[↩︎](#fnref43)
    
44.  http://www.unpbook.com/src.html[↩︎](#fnref44)
    
45.  https://www.openssl.org/[↩︎](#fnref45)
    
46.  https://stackoverflow.com/questions/21323023/[↩︎](#fnref46)
    
47.  https://www.iana.org/assignments/port-numbers[↩︎](#fnref47)
    
48.  https://www.iana.org/assignments/port-numbers[↩︎](#fnref48)
    
49.  https://beej.us/guide/url/unixnet1[↩︎](#fnref49)
    
50.  https://beej.us/guide/url/unixnet2[↩︎](#fnref50)
    
51.  https://beej.us/guide/url/intertcp1[↩︎](#fnref51)
    
52.  https://beej.us/guide/url/tcpi1[↩︎](#fnref52)
    
53.  https://beej.us/guide/url/tcpi2[↩︎](#fnref53)
    
54.  https://beej.us/guide/url/tcpi3[↩︎](#fnref54)
    
55.  https://beej.us/guide/url/tcpi123[↩︎](#fnref55)
    
56.  https://beej.us/guide/url/tcpna[↩︎](#fnref56)
    
57.  https://beej.us/guide/url/advunix[↩︎](#fnref57)
    
58.  https://cis.temple.edu/~giorgio/old/cis307s96/readings/docs/sockets.html[↩︎](#fnref58)
    
59.  https://developerweb.net/?f=70[↩︎](#fnref59)
    
60.  http://www.faqs.org/faqs/internet/tcp-ip/tcp-ip-faq/part1/[↩︎](#fnref60)
    
61.  https://tangentsoft.net/wskfaq/[↩︎](#fnref61)
    
62.  https://en.wikipedia.org/wiki/Berkeley\_sockets[↩︎](#fnref62)
    
63.  https://en.wikipedia.org/wiki/Internet\_Protocol[↩︎](#fnref63)
    
64.  https://en.wikipedia.org/wiki/Transmission\_Control\_Protocol[↩︎](#fnref64)
    
65.  https://en.wikipedia.org/wiki/User\_Datagram\_Protocol[↩︎](#fnref65)
    
66.  https://en.wikipedia.org/wiki/Client-server[↩︎](#fnref66)
    
67.  https://en.wikipedia.org/wiki/Serialization[↩︎](#fnref67)
    
68.  https://www.rfc-editor.org/[↩︎](#fnref68)
    
69.  https://tools.ietf.org/html/rfc1[↩︎](#fnref69)
    
70.  https://tools.ietf.org/html/rfc768[↩︎](#fnref70)
    
71.  https://tools.ietf.org/html/rfc791[↩︎](#fnref71)
    
72.  https://tools.ietf.org/html/rfc793[↩︎](#fnref72)
    
73.  https://tools.ietf.org/html/rfc854[↩︎](#fnref73)
    
74.  https://tools.ietf.org/html/rfc959[↩︎](#fnref74)
    
75.  https://tools.ietf.org/html/rfc1350[↩︎](#fnref75)
    
76.  https://tools.ietf.org/html/rfc1459[↩︎](#fnref76)
    
77.  https://tools.ietf.org/html/rfc1918[↩︎](#fnref77)
    
78.  https://tools.ietf.org/html/rfc2131[↩︎](#fnref78)
    
79.  https://tools.ietf.org/html/rfc2616[↩︎](#fnref79)
    
80.  https://tools.ietf.org/html/rfc2821[↩︎](#fnref80)
    
81.  https://tools.ietf.org/html/rfc3330[↩︎](#fnref81)
    
82.  https://tools.ietf.org/html/rfc3493[↩︎](#fnref82)
    
83.  https://tools.ietf.org/html/rfc3542[↩︎](#fnref83)
    
84.  https://tools.ietf.org/html/rfc3849[↩︎](#fnref84)
    
85.  https://tools.ietf.org/html/rfc3920[↩︎](#fnref85)
    
86.  https://tools.ietf.org/html/rfc3977[↩︎](#fnref86)
    
87.  https://tools.ietf.org/html/rfc4193[↩︎](#fnref87)
    
88.  https://tools.ietf.org/html/rfc4506[↩︎](#fnref88)
    
89.  https://tools.ietf.org/rfc/[↩︎](#fnref89)
